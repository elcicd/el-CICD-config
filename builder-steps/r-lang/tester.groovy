...
def test(def projectInfo, def module) {
    sh """
        Rscript ${el.cicd.BUILDER_STEPS_DIR}/${module.codeBase}/resources/Rlint.R
        mkdir -p ./tests
        if [[ `ls -l ./tests/*.R | wc -l` -gt 0 ]]
        then
            echo "Found tests available to execute"
            Rscript ${el.cicd.BUILDER_STEPS_DIR}/${module.codeBase}/resources/Rtest.R
            badString='path="/'
            goodString='path="./'
